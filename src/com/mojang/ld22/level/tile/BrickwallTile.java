...
{
    public BrickwallTile(final int id) {
        super(id);
    }
    
    @Override
    public void render(final Screen screen, final Level level, final int x, final int y) {
        final int col = Color.get(220, 500, 490, 320);
        final int transitionColor = Color.get(220, 500, 510, level.dirtColor);
        ...
        if (!u && !l) {
            if (!ul) {
                screen.render(x * 16 + 0, y * 16 + 0, 833, transitionColor, 3);
            }
            else {
            ...
            screen.render(x * 16 + 0, y * 16 + 0, (l ? 2 : 1) + (u ? 27 : 26) * 32, transitionColor, 3);
        }
        if (!u && !r) {
            if (!ur) {
                screen.render(x * 16 + 8, y * 16 + 0, 833, transitionColor, 3);
            }
            else {
            ...
            screen.render(x * 16 + 8, y * 16 + 0, (r ? 0 : 1) + (u ? 27 : 26) * 32, transitionColor, 3);
        }
        if (!d && !l) {
            if (!dl) {
                screen.render(x * 16 + 0, y * 16 + 8, 833, transitionColor, 3);
            }
            else {
            ...
            screen.render(x * 16 + 0, y * 16 + 8, (l ? 2 : 1) + (d ? 25 : 26) * 32, transitionColor, 3);
        }
        if (!d && !r) {
            if (!dr) {
                screen.render(x * 16 + 8, y * 16 + 8, 833, transitionColor, 3);
            }
            else {
            ...
            screen.render(x * 16 + 8, y * 16 + 8, (r ? 0 : 1) + (d ? 25 : 26) * 32, transitionColor, 3);
        }
    ...
    public boolean interact(final Level level, final int xt, final int yt, final Player player, final Item item, final int attackDir) {
        if (item instanceof ToolItem) {
            ...
            if (tool.type == ToolType.hammer && tool.level > 0 && player.stamina >= 6 - tool.level) {
                player.payStamina(6 - tool.level);
                level.add(new ItemEntity(new ResourceItem(Resource.stoneplank), xt * 16, yt * 16));
