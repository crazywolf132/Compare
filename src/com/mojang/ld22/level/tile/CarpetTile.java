...
{
    public CarpetTile(final int id) {
        super(id);
    }
    
    @Override
    public void render(final Screen screen, final Level level, final int x, final int y) {
        final int color = Color.get(level.dirtColor, 500, 510, 111);
        final int xt = 21;
    ...
    public boolean interact(final Level level, final int xt, final int yt, final Player player, final Item item, final int attackDir) {
        if (item instanceof ToolItem) {
            ...
            if (tool.type == ToolType.hammer && tool.level >= 0) {
                ...
                level.add(new ItemEntity(new ResourceItem(Resource.carpet), xt * 16, yt * 16));
                level.setTile(xt, yt, Tile.woodfloor, 0);
            }
        }
        return false;
    }
