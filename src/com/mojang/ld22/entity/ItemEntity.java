...
public class ItemEntity extends Entity
{
    ...
    public ItemEntity(final Item item, final int x, final int y) {
        ...
        this.lifeTime = 1200 + this.random.nextInt(60);
    }
    ...
    public void render(final Screen screen) {
        if (this.time == 0) {
            final int mpstate = screen.game.mpstate;
            final int mpstate2 = screen.game.mpstate;
        }
    ...
    public void take(final Player player) {
        ...
        if (this.item instanceof ResourceItem && ((ResourceItem)this.item).resource == Resource.wood) {
            Achievement.wood.Done(player.game);
        }
        if (player.game.mpstate == 1) {
            player.game.tookdrops = true;
        }
        if (player.game.mpstate == 2) {
            player.game.server.dropsupdated = true;
        }
        this.remove();
    }
}
