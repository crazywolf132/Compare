...
public class NewCharacterMenu extends Menu
{
    ...
    public NewCharacterMenu(final boolean flag) {
        this.c1 = new int[3];
        this.c2 = new int[3];
        this.c3 = new int[3];
        ...
        final int b = OptionFile.b % 1000;
        final int c = OptionFile.c % 1000;
        final int d = OptionFile.d % 1000;
        for (int i = 0; i < 3; ++i) {
            if (i == 0) {
                this.c1[i] = b / 100;
                this.c2[i] = c / 100;
    ...
    public void tick() {
        ...
        if (this.ok && this.input.attack.clicked && this.game != null) {
            if (this.game.player != null) {
                OptionFile.a = -1;
                OptionFile.b = this.getC(this.c1);
                OptionFile.c = this.getC(this.c2);
                OptionFile.d = this.getC(this.c3);
                OptionFile.color = Color.get(-1, this.getC(this.c1), this.getC(this.c2), this.getC(this.c3));
                OptionFile.name = this.tb[0].getText();
        ...
        if (this.selected == 0) {
            ...
            if (this.input.right.clicked && !this.tb[this.selected].getEnabled() && !this.ok) {
                this.ok = true;
            }
            if (this.input.left.clicked && !this.tb[this.selected].getEnabled() && this.ok) {
                this.ok = false;
            }
        }
        if (this.selected == 1) {
